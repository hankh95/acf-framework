...
def run_query(sparql: str, data_dir: str | None, as_json: bool):
    ...
    except Exception as e:  # noqa: BLE001 — user-supplied SPARQL; a parse or
        # evaluation error is expected input, not a defect. Narrowing this to
        # rdflib's exception types would let the rest surface as a traceback,
        # which is the wrong answer to a typo in a query.
...
def score(data_path: str, system_id: str | None, as_json: bool, save: str | None):
    ...
    console.print(f"  Dimensions scored: {len(dim_scores)}/12")
    if unmapped:
